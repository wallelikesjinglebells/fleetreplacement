scenarios = {
    "SQ":  dict(n_years=6,  capex_bet_f=1.0,  price_kwh_f=1.0,   capex_dt_f=1.0,  res_dt=0.27, res_bet=0.14, res_bat=0.14, i=0.025, diesel_f=1.0, energy_f=1.0,  toll_dt_f=1.0, toll_bet_mult=1, toll_bet_share=0.00, maint_f=1.0,   tax_f=1.0, ins_f=1.0, wage_f=1.0, eff_dt=1.0,  eff_bet=1.0,  max_km=1300000, max_age=10, bat_repl_age=7),
    "S1":  dict(n_years=5,  capex_bet_f=1.2,  price_kwh_f=1.25,  capex_dt_f=1.1,  res_dt=0.27, res_bet=0.14, res_bat=0.14, i=0.030, diesel_f=1.0, energy_f=1.2,  toll_dt_f=1.0, toll_bet_mult=0, toll_bet_share=1.00, maint_f=1.15,  tax_f=1.2, ins_f=1.0, wage_f=1.0, eff_dt=1.1,  eff_bet=1.25, max_km=1300000, max_age=10, bat_repl_age=7),
    "S2":  dict(n_years=7,  capex_bet_f=0.9,  price_kwh_f=0.875, capex_dt_f=0.95, res_dt=0.20, res_bet=0.34, res_bat=0.34, i=0.030, diesel_f=1.0, energy_f=1.0,  toll_dt_f=1.0, toll_bet_mult=0, toll_bet_share=0.75, maint_f=0.925, tax_f=1.0, ins_f=1.0, wage_f=1.0, eff_dt=0.95, eff_bet=0.875,max_km=1560000, max_age=11, bat_repl_age=7),
    "S3":  dict(n_years=5,  capex_bet_f=1.1,  price_kwh_f=1.125, capex_dt_f=1.05, res_dt=0.27, res_bet=0.14, res_bat=0.14, i=0.020, diesel_f=1.1, energy_f=1.0,  toll_dt_f=1.1, toll_bet_mult=0, toll_bet_share=0.50, maint_f=1.075, tax_f=1.0, ins_f=1.0, wage_f=1.0, eff_dt=1.05, eff_bet=1.125,max_km=1300000, max_age=10, bat_repl_age=7),
    "S4":  dict(n_years=7,  capex_bet_f=0.8,  price_kwh_f=0.75,  capex_dt_f=0.9,  res_dt=0.20, res_bet=0.34, res_bat=0.34, i=0.020, diesel_f=1.1, energy_f=0.8,  toll_dt_f=1.1, toll_bet_mult=0, toll_bet_share=0.25, maint_f=0.85,  tax_f=1.0, ins_f=1.0, wage_f=1.0, eff_dt=0.9,  eff_bet=0.75, max_km=1560000, max_age=11, bat_repl_age=7),
}
...
def subsidy(s):
    ...
    return min(prem * subsidy_perc, subsidy_max)
...
def opex_bet(s, age, bat_repl=False):
    ...
    maint = akt_base * maint_bet * s["maint_f"] * (1 + maint_age_bet * age)
    hours = akt_base / avg_speed
    drv   = hours * driver_wage * s["wage_f"]
    ins   = ins_base * s["ins_f"]
    t     = tax * s["tax_f"]
    bat_r = battery_cost(s) if bat_repl else 0
    return dict(fuel=fuel, toll=toll, maint=maint, driver=drv, ins=ins, tax=t, bat_repl=bat_r,
                total=fuel + toll + maint + drv + ins + t + bat_r)


# CHECK 1: Annual OPEX comparison DT vs BET at age 1
print("=" * 80)
print("CHECK 1: Annual OPEX — DT vs BET at age 1 (no battery replacement)")
print("=" * 80)
...
# CHECK 5: Battery replacement trigger vs effective max age
print("=" * 80)
print("CHECK 5: Battery replacement — does it ever trigger?")
print("=" * 80)
print(f"  {'':20} {'max_km':>10} {'km-cap age':>12} {'bat_repl_age':>14}  verdict")
print(f"  {'-'*68}")
for name, s in scenarios.items():
    km_age = s["max_km"] / akt_base
    triggers = s["bat_repl_age"] < km_age
    verdict = "YES" if triggers else "NO — mileage forces retirement first"
    print(f"  {name:<20} {s['max_km']:>10,.0f} {km_age:>12.1f} {s['bat_repl_age']:>14}  {verdict}")


# CHECK 6: Total undiscounted lifetime cost — hold from new until forced retirement
print("\n" + "=" * 80)
print("CHECK 6: Total undiscounted cost — hold new vehicle until mileage limit")
print("  (BET includes first charger install; DT salvage and BET salvage at end)")
print("=" * 80)
...
for name, s in scenarios.items():
    ...
    for age in range(1, max_life + 1):
        bat_r = (age == s["bat_repl_age"])
        bet_total += opex_bet(s, age, bat_repl=bat_r)["total"]
